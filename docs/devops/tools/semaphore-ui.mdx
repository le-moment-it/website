...
sequenceDiagram
    ...
    participant Server
    Semaphore->>Git: Pull repository
    Semaphore->>Server: Apply configuration
```
...
![Overview](/img/devops/tools/semaphore-ui/overview.png)
...
![Task-Template](/img/devops/tools/semaphore-ui/task-template.png)
  </TabItem>
  <TabItem value="repository" label="Repository" default>
export const configuration = (() => {
  const main = `
# Main entrypoint for semaphore-ui task launch
- name: Run Ansible
  hosts: servers
  tasks:
...
servers:
  hosts:
    semaphore:
      ansible_host: semaphore-ui.stream.vasseurlaurent.com
      ansible_user: root
`.trim();
...
return [
    {
      ...
      children: [
...

        {
          ...
          type: 'directory',
          children: [
            { name: 'backend.tf', type: 'file', content: 'Not relevant ...' },
            { name: 'data.tf', type: 'file' , content: 'Not relevant ...'},
            { name: 'provider.tf', type: 'file' , content: 'Not relevant ...'},
            { name: 'server.tf', type: 'file' , content: 'Not relevant ...'},
            { name: 'variables.tf', type: 'file' , content: 'Not relevant ...'},
          ]
        },
        { name: 'README.md', type: 'file' , content: '# Semaphore repository'},
        { name: '.gitlab-ci.yml', type: 'file' , content: 'Not relevant ...'},
      ],
    }
  ];
})();
...
![View](/img/devops/tools/semaphore-ui/view.png)
...
![Output](/img/devops/tools/semaphore-ui/variable-group.png)
...
![Output](/img/devops/tools/semaphore-ui/integration.png)
...
```
https://semaphore.stream.vasseurlaurent.com/api/integrations/czep2z24okm8x2jb
```
...
<Tabs>
  <TabItem value="advantage" label="✅ Advantages" default>
  ...
  <TabItem value="disadvantages" label="❌ Disadvantages" default>

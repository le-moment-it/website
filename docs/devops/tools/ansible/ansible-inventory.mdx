...
description: Create dynamic inventory
---
...
<Terminal  prompt="le-moment-it@laptop:~$"
  title="Install ansible collection"
  lines={[
    "mkdir ansible-galaxy collection install hetzner.hcloud"
  ]}
/>
...
| Name | Labels |
|-----------|-----------|
| atlantis   | `environment=prod` `product=atlantis` `team=devops`  |
| semaphore   | `environment=prod` `product=semaphore` `team=devops`  |
...
```yaml title="./configuration/hcloud.yml"
...
<Terminal  prompt="le-moment-it@laptop:~$"
  title="Launch ansible-inventory"
  lines={[
...
```yaml title="dynamic_inventory.yml"
all:
  children:

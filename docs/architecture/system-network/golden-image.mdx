...
<Admonition type="note" icon="💡" title="But also ...">
...
As soon as a new machine will be instanciated with this golden image, it will automatically download the website from the S3 and will be ready to serve traffic 🚀
...
When you develop a new golden image, you might want to test that this image is answering your need. To do so, you can create multiple kind of tests :

- Integration tests : Even if `packer` is not designed to perform tests, you may want to add integration tests inside your building process to ensure the quality of your golden image. A simple way to add them is to create an ansible playbook, that will be called with the `ansible provisioner` (in last position) that will perform readable and understandable tests for you.

:::info
By default, Ansible allows you to easily perform tests with module such as [ansible.builtin.assert](https://docs.ansible.com/ansible/latest/collections/ansible/builtin/assert_module.html)
:::


- End to End (often call E2E) tests : These tests can be implemented only if your golden image is deployed in one of your testing environment.

## Conclusion

Golden images are key resources when you want to deploy virtual machines dynamically using a horizontal scaling system (such as AWS autoscaling group). However, its development can be a tedious task.

Creating an efficient golden image development process through infrastructure as code is required to ensure the reusability and the maintanability of your golden images.